...
Using **all** did the work, as I later analyzed that the code also uses **len** to check the length of the code sent, before potentially rejecting the input. If **len** is overwritten by **all**, the line that checks the length will return false, since the output of **all(code)** would be **1**, which is always less than 5.
